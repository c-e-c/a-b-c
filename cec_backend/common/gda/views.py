...
#         ),
#     ]))
def gda_list(request, format=None):
    '''
    type为表名（必选）如'SysParamType'，
    props（可选）为要查询的列名数组
    filters（可选）为过滤条件数组，其中column为过滤的列，
    ...
    # 设置queryset
    Table = tableClass(params.get('type'))
    retval = None
    if params.get('filters') is not None:
        filters = json.loads(params.get('filters'))
        d = dict()
        for filter in filters:
            ...
            if filter['comparison'] == 'isnull':
                ...
                v = None
            d[k] = v
        retval = Table.objects.filter(**d).values(
            *json.loads(params.get('props'))
        )
    else:
        retval = Table.objects.all().values(*json.loads(params.get('props')))

    page_size = params.get('limit')
    if page_size is not None:
        p = Paginator(retval, int(page_size))
        page_offset = params.get('offset') if params.get('offset') else 0
        if int(page_offset) < p.count:
            retval = p.get_page(int(page_offset) // int(page_size)).object_list
        else:
            pass
    else:
        pass
